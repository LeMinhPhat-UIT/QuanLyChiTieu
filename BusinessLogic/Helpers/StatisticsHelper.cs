    ...
    internal class StatisticsHelper
    {
        public static double GetIncome(DateTime startDate, DateTime endDate)
            => FinanceService._allTransaction
                .Where(x => x.TransactionType == MoneyFlow.Income && DateHelper.isBetween(x.TransactionDate, startDate, endDate))
                .Select(x => x.Money).Sum();

        public static double GetExpense(DateTime startDate, DateTime endDate)
            => FinanceService._allTransaction
                .Where(x => x.TransactionType == MoneyFlow.Expense && DateHelper.isBetween(x.TransactionDate, startDate, endDate))
                .Select(x => x.Money).Sum();
        ...
        public static Chart GetDataForChart(MoneyFlow TransactionType, DateTime startDate, DateTime endDate)
        {
            Chart chart = new Chart();

            List<Transaction> data = FinanceService.GetAllDataByFlow(TransactionType, startDate, endDate);
